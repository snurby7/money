...
export class CategoryService
  ...
  public findCategories(
    ...
    const { query, params } = categoryQueries.getAllCategoriesByBudget(
      budgetId
    );
  ...
  public getCategoryWithChildren(
    ...
    const { query, params } = categoryQueries.getCategoryWithChildrenById(
      id,
      budgetId
    );
    return this.neo4jService.rxSession.readTransaction((trx) =>
   ...
   * @memberof CategoryService
   */
  private createTopLevelCategory(
    request: ICreateCategory
  ): Observable<ICategory> {
    const resultKey = 'category';
    const { query, params } = categoryQueries.createCategory(
      resultKey,
      request
    );
    return this.neo4jService.rxSession.writeTransaction((trx) =>
      trx
        .run(query, params)
        .records()
        .pipe(getRecordsByKey<ICategory>(resultKey))
    );
  }
  ...
  private createChildCategory(request: ICreateCategory): Observable<ICategory> {
    ...
    const { query, params } = categoryQueries.createChildCategory(
      resultKey,
      request
    );
    return this.neo4jService.rxSession.writeTransaction((trx) =>
  ...
  private getCategoryById(
    ...
    const resultKey = 'category';
    const { query, params } = categoryQueries.getCategoryNode(
      resultKey,
      categoryId,
      budgetId
    );
